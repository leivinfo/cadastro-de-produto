...
// Verifica se existe PATH_INFO (o que vem depois do .php)
if (isset($_SERVER['PATH_INFO'])) {
    $rota = trim($_SERVER['PATH_INFO'], '/');
} else {
    // Fallback: Tenta limpar o nome do script da URL manualmente
    $uri = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
    $script = $_SERVER['SCRIPT_NAME']; // Ex: /api.php
    
    // Se a URI começar com /api.php, remove essa parte
    if (strpos($uri, $script) === 0) {
        $rota = substr($uri, strlen($script));
    } else {
        $rota = $uri;
    }
    // Remove barras do inicio e fim
    $rota = trim($rota, '/');
}
